...
def modified_modularity_node(g,community_map_to_node,node_map_to_community,node_v):
    '''
    ...
    for node in neighbor_set:
        for community in node_map_to_community[node]:
    ...
    for community in node_map_to_community[node_v]:
        "Get the inner node set of current community for node_v"
        inner_node_set = neighbor_set.intersection(community_map_to_node[community])
        ...
        for node_e in external_node_set:
            for community_e in node_map_to_community[node_e]:
        ...
        for node_i in community_map_to_node[community]:
        ...
        if e_max[0] != -1:
            for node_i in neighbor_set:
                if node_i in community_map_to_node[e_max[0]]:
                    modularity_for_current_community = modularity_for_current_community - (1 - (degree_v * len(g.graphlist[node_i]))/ float(2 * edge_count)) 
                    """
...
def modularity(g,community_map_to_node,node_map_to_community):
    '''
    ...
    '''
    modularity = 0
    if(len(community_map_to_node) == 1):
        return 0
    edge_count = g.edge_count
    for community in community_map_to_node.iterkeys():
        "W.r.t each community, calculate the modularity"
        node_set_in_community = community_map_to_node[community]
